...
########################################
# 1) 项目根 AGENTS.md（短版，通用）
########################################
if [[ -f "$ROOT_AGENTS_FILE" ]]; then
  echo "⚠️  检测到已存在 $ROOT_AGENTS_FILE，保持原样，跳过写入。"
else
...
EOF
  ...
  echo "✅ 已生成项目根指令文件: $ROOT_AGENTS_FILE"
fi

########################################
# 2) .agent/AGENTS.md（更完整说明）
########################################
if [[ -f "$AGENTS_FILE" ]]; then
  echo "⚠️  检测到已存在 $AGENTS_FILE，保持原样，跳过写入。"
else
  mkdir -p "$AGENT_DIR"
  cat > "$AGENTS_FILE" << 'EOF'
...
EOF
  ...
  echo "✅ 已生成仓库内部指令文件: $AGENTS_FILE"
fi

########################################
# 3) .agent/template/PLAN.md（ExecPlan 模板 + 融合脑暴约束）
########################################
if [[ -f "$PLAN_FILE" ]]; then
  echo "⚠️  检测到已存在 $PLAN_FILE，保持原样，跳过写入。"
else
  mkdir -p "$TEMPLATE_DIR"
  cat > "$PLAN_FILE" << 'EOF'
...
EOF
  ...
  echo "✅ 已生成 ExecPlan 模板文件: $PLAN_FILE"
fi

echo "🎉 完成：已为 $TOOL 初始化 Brainstorming + ExecPlan 规范（存在的文件已自动跳过）。"
